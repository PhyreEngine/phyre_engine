...
class FastaParser(Component):
    ...
    Alignments will be stored in the ``fasta_alignment`` key of each element in
    the ``templates`` list. If no aligment was generated for a hit or that
    alignment could not be parsed, ``fasta_alignment`` will be set to `None`.
    """
    ...
    def run(self, data, config=None, pipeline=None):
        ...
        for template, hit in zip(templates, parsed_alignments.hits):
            ...
            tname = template["name"] if "name" in template else "Template"
            template["fasta_alignment"] = textwrap.dedent("""\
            >{query_name}
            {query_seq}
            >{template_name}
            {template_seq}
            """).format(
                query_name=qname, query_seq=query_seq,
                template_name=tname, template_seq=template_seq)
        return data
