...
class TMAlign(object):
    ...
    :ivar invert: (Default: `False`) Whether to invert the order of arguments
        when calling the TMalign executable. This only affects the class
        internally: aligning *a* to *b* will give the same results in the same
        order whether or not this variable is true. This only affects the raw
        output of TMalign and the order in which structures appear in any
        superpositions. It may be useful to set this variable when aligning
        lots of structures to a reference structure because TMalign always
        rotates the first structure onto the second, so superpositions produced
        in this way will all be consistent with the reference structure.
    ...
    def align(self, file1, file2, superpos=None):
        ...
        :param str superpos: Prefix of superposition files to generate, or
            `None` if no superposition is to be written. Files will be
            generated with the standard suffixes generated by TMalign with the
            ``-o`` option.
